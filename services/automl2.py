...
def build_model(df):
    ...
    df_categorical = pd.get_dummies(categorical)
    df = pd.concat([df,df_categorical], axis = 1)
    df = df.drop(categorical, axis = 1)
    
    Y = df[selected_target]
    X = df.iloc[:,:]  #if x is defined as all columns except the response, then x is not required
    X.drop(columns = [selected_target], inplace = True)
